                                        ...
                                        <div class="col-6 pb-3">
                                            <div class="card card-pricing popular text-center px-3 mb-5 mb-lg-0">
                                                ...
                                                <div class="card-body delimiter-top">
                                                    ...
                                                    @if(\Auth::user()->type != 'admin')
                                                        @if(\Auth::user()->plan != $plan->id && !empty(env('STRIPE_KEY')) && !empty(env('STRIPE_SECRET')))
                                                            <button class="btn btn-sm btn-neutral mb-3">
                                                                <a href="{{route('payment',\Illuminate\Support\Facades\Crypt::encrypt($plan->id))}}">{{__('Subscribe')}}</a>
                                                            </button>
                                                        @elseif(empty(\Auth::user()->plan_expire_date))
